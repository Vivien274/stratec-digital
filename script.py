...
if not os.path.exists(txt_log_file):
    print(f"Erreur : fichier {txt_log_file} introuvable !")
...
def analyze_ips(ip_list, threshold=5):
...
def block_ip(ip):
    with open(blocked_ips_file, "a") as f:
        f.write(ip + "\n")
    print(f"IP {ip} ajoutée à blocked_ips.txt")

# Exécution du script
if __name__ == "__main__":
    ips = extract_ips(txt_log_file)
    # print(f"IPs extraites : {ips}")
    all_ips, suspicious_ips = analyze_ips(ips)
    print(f"IPs analysées : {all_ips}")
    print(f"IPs suspectes : {suspicious_ips}")
