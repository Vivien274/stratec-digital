...
if( ! empty( $_POST['email'] ) ) {
	...
	$receiver_name 	= 'Stéphanie • Stratec-Digital';
	...
	if( $enable_mailchimp == 'no' ) { // Simple / SMTP Email
		...
		$api_key 	= getenv('MAILCHIMP_API_KEY'); // Your MailChimp API Key
		$list_id 	= 'a323c9b9f3'; // Your MailChimp List ID
		$status 	= 'subscribed';
		...
		if ( ! empty( $result->status ) AND $result->status == 'subscribed' ) {
			...
			if( ! empty( $redirect_page_url ) ) {
		   	...
		   	//Success Message
			echo '{ "alert": "alert-success", "message": "Vous vous êtes bien abonné(e) à notre Newsletter ! Merci !" }';
		} else {
			//Fail Message
			echo '{ "alert": "alert-danger", "message": "Votre message n\'a pas pu être envoyé." }';
		}
	}
} else {
	//Empty Email Message
	echo '{ "alert": "alert-danger", "message": "Merci de saisir une adresse email" }';
}
