...
import { Save, CheckCircle2, Star, Loader2, CreditCard, Tag, ListChecks } from "lucide-react";

// Helper to parse features json string to multiline text for editing
function getFeaturesText(featuresRaw: string): string {
  try {
...
function PackCardItem({ pack: initialPack, index }: { pack: Pack; index: number }) {
  const [pack, setPack] = useState<Pack>(initialPack);
  const [featuresText, setFeaturesText] = useState(() => getFeaturesText(initialPack.features));
        ...
        <div className="flex items-center gap-4">
          ...
          <label className="flex items-center gap-2 text-xs font-bold text-[#562C2C] cursor-pointer bg-[#F5DFBB]/40 hover:bg-[#F5DFBB]/80 px-3 py-1.5 rounded-xl border border-[#562C2C]/10 transition-colors">
            <input
              ...
              onChange={(e) => setPack({ ...pack, popularBadge: e.target.checked })}
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            type="text"
            value={pack.audience || ""}
            onChange={(e) => setPack({ ...pack, audience: e.target.value })}
        ...
        <div>
          ...
          <input
            type="text"
            value={pack.tagline || ""}
            onChange={(e) => setPack({ ...pack, tagline: e.target.value })}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setPack({ ...pack, billingPeriod: e.target.value })}
              className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 focus:ring-2 focus:ring-[#0E9594] text-slate-700 text-xs"
        ...
        <div className="pt-1">
          <label className="flex items-center gap-2.5 text-xs font-bold text-[#562C2C] cursor-pointer bg-[#FAF4F2] p-3 rounded-xl border border-[#562C2C]/10">
            <input
              ...
              onChange={(e) => setPack({ ...pack, allowSplitPayment: e.target.checked })}
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
      {initialPacks.map((pack, index) => (
        <PackCardItem key={pack.id} pack={pack} index={index} />
      ))}
    </div>
  );
}
